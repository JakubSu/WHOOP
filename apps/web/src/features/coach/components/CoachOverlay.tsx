...
export function CoachOverlay() {
  ...
  return <div aria-live={chat.isStreaming ? 'polite' : 'off'}>
    {!sheet.isOpen ? <CoachCollapsedBar label={label} isStreaming={chat.isStreaming} onOpen={openCoach} /> : null}
    <CoachBottomSheet open={sheet.isOpen} onOpenChange={(open) => open ? openCoach() : sheet.close()} onDragStart={sheet.onDragStart} onDragEnd={sheet.onDragEnd}>
      <CoachSheetHeader label={label} conversations={chat.conversations} conversationId={chat.conversationId} isBusy={isBusy} onSelect={(conversationId) => void chat.selectConversation(conversationId)} onNewChat={() => void chat.newChat()} onCollapse={sheet.close} />
      <CoachMessageList scrollRef={autoScroll.scrollRef} messages={chat.messages} activeMessageId={chat.activeMessageId} thinking={chat.thinking} isLoading={chat.isLoading} error={chat.error} canLoadOlder={Boolean(chat.nextMessageCursor)} isFollowing={autoScroll.isFollowing} onScroll={autoScroll.onScroll} onLoadOlder={() => void chat.loadOlderMessages()} onJumpToLatest={() => autoScroll.scrollToLatest()} />
      <CoachComposer value={chat.input} isLoading={chat.isLoading} isStreaming={chat.isStreaming} onChange={chat.setInput} onSubmit={() => void chat.send()} />
    </CoachBottomSheet>
  </div>
}
