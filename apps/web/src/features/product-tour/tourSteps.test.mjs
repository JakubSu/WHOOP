...
test('guided Coach steps target the user actions in the recommendation flow', () => {
  ...
  assert.equal(typeof acceptStep?.onHighlightStarted, 'function')
  assert.equal(acceptStep?.onHighlighted, undefined)
  ...
  assert.equal(typeof replacementStep?.onHighlightStarted, 'function')
  ...
  assert.equal(typeof mobileAcceptStep?.onHighlightStarted, 'function')
...
test('accounts without workouts receive a first-workout onboarding tour', () => {
  ...
  assert.deepEqual(emptySteps[4]?.popover?.showButtons, ['next', 'close'])
