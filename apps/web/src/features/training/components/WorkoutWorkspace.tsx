...
export function WorkoutWorkspace() {
  ...
  async function handleDelete() {
    ...
    navigate(workoutPage.workout?.date ? `/week?date=${workoutPage.workout.date}` : '/week')
  }
  ...
  return <section className="mx-auto grid min-h-0 w-full max-w-2xl grid-rows-[auto_1fr] px-4 py-6 sm:px-6 lg:max-w-none lg:px-8 lg:py-7" data-tour-workspace-ready={!workoutPage.isLoading ? 'true' : undefined}>
    ...
    <div className="grid min-h-0 grid-rows-[auto_auto_1fr]" data-tour="workout-panel">
      <WorkoutHeader canDelete={Boolean(workoutPage.resolvedWorkoutId)} canEdit={Boolean(workoutPage.resolvedWorkoutId)} draftWorkoutName={editor.draftWorkoutName} exerciseCount={visibleExercises.length} isDeleting={workoutPage.isDeleting} isEditing={editor.isEditing} isLoading={workoutPage.isLoading} isSaving={editor.isSaving} isToday={workoutPage.isToday} nextWorkout={workoutPage.nextWorkout} previousWorkout={workoutPage.previousWorkout} workout={workoutPage.workout} onCancelEditing={editor.cancelEditing} onDelete={handleDelete} onNext={() => workoutPage.nextWorkout && navigate(`/workouts/${workoutPage.nextWorkout.id}`)} onPrevious={() => workoutPage.previousWorkout && navigate(`/workouts/${workoutPage.previousWorkout.id}`)} onSave={editor.save} onStartEditing={editor.startEditing} onWorkoutNameChange={editor.setDraftWorkoutName} />
      ...
      <InlineError message={workoutPage.deleteError ? getErrorMessage(workoutPage.deleteError) : null} />
      </div>
