...
export function AddExerciseDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => nextOpen ? onOpenChange(true) : close()}>
      <DialogContent className="z-[60] max-h-[calc(100dvh-2rem)] overflow-y-auto" aria-describedby={undefined}>
        ...
        {step === 'create' ? (
          <form className="mt-4 space-y-4" onSubmit={handleCreate}>
            ...
            <div className="flex justify-end gap-2 pt-2"><Button type="button" variant="outline" disabled={isCreating} onClick={() => setStep('search')}>Back</Button><Button type="submit" disabled={isCreating} data-tour="create-exercise-submit">{isCreating ? 'Creating…' : 'Create exercise'}</Button></div>
          </form>
        ) : null}
